...
export function removeItem(db, itemId) {
  return db.prepare('DELETE FROM items WHERE item_id = ?').run(itemId).changes;
}
...
export function transactionCoverage(db) {
  return db.prepare(`
    SELECT
      i.item_id,
      i.plaid_env,
      i.institution_name,
      i.cursor IS NOT NULL AS has_cursor,
      i.last_synced_at,
      i.needs_update,
      i.last_error_code,
