...
export function migrate(db = openDb()) {
  ...
  if (!itemColumns.includes('plaid_env')) {
    db.exec("ALTER TABLE items ADD COLUMN plaid_env TEXT NOT NULL DEFAULT 'sandbox'");
  }
...
export function listItems(db) {
  return db.prepare(`
    SELECT item_id, plaid_env, institution_name, cursor IS NOT NULL AS has_cursor, last_synced_at
    FROM items
